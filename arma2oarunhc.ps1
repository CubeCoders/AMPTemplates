...
for ($i = 1; $i -le [int]$args[0]; $i++) {
  ...
  $hcProcess = Start-Process -FilePath "arma2oaserver.exe" -ArgumentList "-client", "-nosound", "-profiles=a2aomaster", "-connect=${connect}:$($args[2])", "-port=$basePort", "-password=`"$($args[3])`"", "`"-mod=$($args[4])`"", "`"-par=$parfile`"" -WindowStyle Hidden -PassThru
